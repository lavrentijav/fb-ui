...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: 'Flip one registered panel between the two roles of the same row: a node
        this panel controls, or a master it advertises as a subscription
        fallback. Send only what the target role needs — address/port/apiToken
        for node, subDomain/subPort/subPath for master; the other role’s fields
        are kept so flipping back does not re-ask them. Observed health (status,
        latency, learned key) is reset, since the two roles are probed by
        different jobs. Refused while inbounds are still attached to the node,
        and for the row that is this panel itself.'
      url: '#flip-one-registered-panel-between-the-two-roles-of-the-same-row-a-node-this-panel-controls-or-a-master-it-advertises-as-a-subscription-fallback-send-only-what-the-target-role-needs--addressportapitoken-for-node-subdomainsubportsubpath-for-master-the-other-roles-fields-are-kept-so-flipping-back-does-not-re-ask-them-observed-health-status-latency-learned-key-is-reset-since-the-two-roles-are-probed-by-different-jobs-refused-while-inbounds-are-still-attached-to-the-node-and-for-the-row-that-is-this-panel-itself'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: 'Flip one registered panel between the two roles of the same row: a
          node this panel controls, or a master it advertises as a subscription
          fallback. Send only what the target role needs — address/port/apiToken
          for node, subDomain/subPort/subPath for master; the other role’s
          fields are kept so flipping back does not re-ask them. Observed health
          (status, latency, learned key) is reset, since the two roles are
          probed by different jobs. Refused while inbounds are still attached to
          the node, and for the row that is this panel itself.'
        id: flip-one-registered-panel-between-the-two-roles-of-the-same-row-a-node-this-panel-controls-or-a-master-it-advertises-as-a-subscription-fallback-send-only-what-the-target-role-needs--addressportapitoken-for-node-subdomainsubportsubpath-for-master-the-other-roles-fields-are-kept-so-flipping-back-does-not-re-ask-them-observed-health-status-latency-learned-key-is-reset-since-the-two-roles-are-probed-by-different-jobs-refused-while-inbounds-are-still-attached-to-the-node-and-for-the-row-that-is-this-panel-itself
